...
/**
 * Generated class for the CreateclockPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-createclock',
  templateUrl: 'createclock.html',
})
export class CreateclockPage {
  ...
  chosePhoto() {
    ...
    this.imagePicker.getPictures(options).then((results) => {
      for (let i = 0; i < results.length; i++) {
        ...
        // alert(results[i]);
        this.path = 'data:image/png;base64,' + results[i];
        this.showPhoto = true;
      }
    }, (err) => {
      alert(err);
  ...
  //发表
  publish() {
    if(this.showPhoto === false || this.textNum === 0){
      let alert = this.alertCtrl.create({
        subTitle: "请上传图片并附上心情哦",
        buttons: ["OK"]
      });
      alert.setMode("ios");
      alert.present();
      ...
      form.append('clockContent', this.textContent);

      let xhr = new XMLHttpRequest();

      let that = this;
      xhr.open('POST', `${this.url}addClock`, true);
      xhr.send(form);
